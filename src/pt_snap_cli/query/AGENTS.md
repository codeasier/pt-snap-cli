...
- When changing template behavior, check YAML templates, `config.py`, `registry.py`, `executor.py`, and CLI template metadata output together. Field semantics ride the same YAML → config → registry → core → API/MCP/CLI path as `choices`; do not add a second schema.
