...
Use the displayed database and focused device only when both are present and
the database file exists. If either remains absent or ambiguous, ask the user
to select it and stop until it is explicit. Do not run `pt-snap focus <database_path>`, use `--session`, or
...
4. After the user confirms a path, the next inspect step is
   `pt-snap metadata "<db_path>" --json`. Do not start with `pt-snap query`.
...
only when the `sqlite3` CLI is available, use this read-only fallback. This
read-only aggregate fallback applies only after a confirmed SnapshotDB path. It
is not available during missing-focus recovery. Scope
