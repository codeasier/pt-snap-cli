...
│                                                              │
│  使用方式：                                                   │
│  - 开发者：直接使用 CLI 命令 或通过 SDK 集成到工作流           │
│  - AI Agent：调用 CLI 命令（解析 JSON）或 直接调用 SDK API    │
└───────────────────────────┬──────────────────────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────┐
│                    Python SDK/API                            │
...
**架构说明**:
- **用户交互层**: 提供 CLI 和 SDK 两种调用接口，AI Agent 和开发者可以**共同使用**
  - CLI: 命令行工具，输出结构化 JSON，适合 AI Agent 解析和开发者快速查看
  - SDK: Python API，适合集成到自动化工作流和 AI Agent 直接调用
- **SDK/API 层**: 核心业务逻辑封装，提供查询、分析、RAG 管理接口
- **数据访问层**: SQLite 数据库、查询配置、SQL 模板
- **知识库层**: 向量数据库、案例归档、学习日志（阶段 3 实现）

**AI 友好设计**:
- CLI 命令默认输出 JSON，AI 可解析
- SDK 提供类型安全的 API，AI 可生成调用代码
- 两种接口功能对等，AI 可根据场景选择
