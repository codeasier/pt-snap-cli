...
`--json` 覆盖读取、设置、仅改设备、`--global` 与 `--session`。成功对象包含 `schema_version`、`ok`、`action`、`configured`、`db_path`、`focus_source`、`focus_file`、`device_id`、`available_devices`、`callstack_layout` 和 `db_exists`。`focus --session --json` 只验证数据库，并返回 `session_applied: false` 以及 `env.name` / `env.value` / `env.export`；不会打印可直接 eval 的裸 shell 行，也不表示已修改父 shell。
