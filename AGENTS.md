...
- Type check: `python -m basedpyright --pythonpath "$(python -c 'import sys; print(sys.executable)')"`
  (covers all of `src/pt_snap_cli`; `pyproject.toml` downgrades a listed set of
  rules to warnings under `src/pt_snap_cli/snapshot/` until its annotations are
  fixed — do not widen that list, shrink it)
