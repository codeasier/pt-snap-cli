...
```bash
pt-snap query <db_path> --template <template_name> [options]
```

**参数说明：**
- `db_path`: SQLite 数据库文件路径（必需）
- `--template`: 查询模板名称（必需，除非使用 --list）
...
指定设备查询：
```bash
pt-snap query snapshot.pkl.db --template active_blocks --device 0
```
...
3. **memory_timeline_v2** - 内存时间线
   ...
   ```bash
   pt-snap query snapshot.pkl.db --template memory_timeline_v2
   ```
