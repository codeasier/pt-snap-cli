...
本次发布聚焦 Agent 诊断技能与查询/导入性能：新增内存泄漏、碎片、峰值拆解与 Ascend NPU 采集等诊断 skill 及本地 skill 评测框架；SnapshotDB 采用去重 callstack 存储，叠加查询执行层优化，使导入、拆分与查询获得数量级加速；pickle 加载收敛到内建类型白名单，进一步降低反序列化风险。
...
- 导入格式版本升级为 2：旧布局的缓存 SnapshotDB 不再复用，升级后首次导入会自动重建。
